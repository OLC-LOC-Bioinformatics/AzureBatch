...
class AzureBatch:
    ...
    def main(self):
        ...
        batch_client = BatchServiceClient(
            credentials, batch_url=self.settings.batch_account_url
        )
        ...
        if not self.unique_id:
            ...
            self.logger.warning("Using %s as the unique identifier", self.unique_id)
        ...
        try:
            ...
            end_time = datetime.datetime.now(tz=datetime.timezone.utc).replace(
                microsecond=0
            )
    ...
    def __init__(
        ...
        # calculate timeouts
        self.start_time = datetime.datetime.now(tz=datetime.timezone.utc).replace(
            microsecond=0
        )
        ...
        self.sys_call = read_command_file(command_file=command_file)
        ...
        self.container = validate_container_name(container_name=container)
...
def cli():
    ...
    parser.add_argument(
        ...
        "e.g. batch.node.ubuntu 24.04\n"
        "NANOPORE_IMAGE=resource ID of the Nanopore VM image\n"
        "NANOPORE_NODE_AGENT_SKU=node agent SKU of the image"
        "e.g. batch.node.ubuntu 24.04\n"
    ...
    parser.add_argument(
        ...
        default="COWBAT",
        choices=["COWBAT", "COWSNPhR", "AmpliSeq", "Nanopore"],
    ...
    local_settings = Settings(
        settings=settings_dict, analysis_type=arguments.analysis_type
    )
